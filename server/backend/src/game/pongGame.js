...
export class Game {
  ...
  resetBall(paddleHit) {
    ...
    if (
      this.score.player1 >= gameProperties.WINNING_SCORE ||
      this.score.player2 >= gameProperties.WINNING_SCORE
    ) {
      this.gameState = "finished";
      this.winner =
        this.score.player1 > this.score.player2 ? "first" : "second";
      return;
    }
    if (!paddleHit) {
      this.ball.z = 0;
      this.ball.x = 0;
      this.ball.speedZ =
        (Math.random() > 0.5 ? 1 : -1) * gameProperties.BALL_SPEED_Z;
    }
    if (paddleHit === "first") {
    ...
    this.ball.speedX =
      (Math.random() > 0.5 ? 1 : -1) * gameProperties.BALL_SPEED_X;
  ...
  update() {
    ...
    if (
      ...
      distanceToPaddle < 0 &&
      distanceToPaddle > -this.ball.radius &&
    ...
    if (
      this.ball.z <
      -gameProperties.GAME_WIDTH / 2 -
        gameProperties.EXTRA_SPACE -
        gameProperties.BALL_SIZE
    ) {
  ...
  getState() {
    ...
    if (this.scoreEvent)
      evs.push({
        ...
        points:
          this.scoreEvent === "first" ? this.score.player1 : this.score.player2,
      });
